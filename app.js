...
con.connect(function(err) {
  if (err) throw err;
  console.log("Connected to Chaddon Database");
});
...
io.sockets.on("connection", function(socket) {
  ...
  socket.on("adduser", function(message) {
    var sql =
      "SELECT * FROM verifieduser WHERE TOKEN = '" + message.token + "'";
  ...
  socket.on("verifyUser", function(data) {
    ...
    var sql =
      "INSERT INTO verifiedUser (name,token) VALUES ('" +
      data +
      "','" +
      token +
      "')";
  ...
  socket.on("verifySend", function(data) {
    var sql = "SELECT * FROM verifieduser WHERE TOKEN = '" + data.token + "'";
    con.query(sql, function(err, result) {
      if (result.length > 0) {
        if (result) {
  ...
  socket.on("message", function(msg) {
    ...
    console.log("message");
    var sql = "SELECT * FROM verifieduser WHERE TOKEN = '" + msg.token + "'";
    con.query(sql, function(err, result) {
      if (result.length > 0) {
        if (result) {
  ...
  socket.on("disconnect", function() {
    ...
    var callId = "First call to update users";
    ...
    console.log("still alive! " + socket.username);
